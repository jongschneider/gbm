---
name: interactive-tmux-session
description: Creates a Tmux session in a temporary directory, displays the session name, and waits for approval before proceeding with interactive commands. Use when you need to run commands in a shared Tmux session that the user will attach to and observe.
---
...
3. Wait for user approval/instruction
4. Execute commands in the session using `tmux send-keys` and `tmux capture-pane`
...
After printing the session name, pause and ask the user to confirm they've attached:

> "Session `$SESSION_NAME` is ready. Attach with: `tmux attach-session -t $SESSION_NAME`. Reply when you're ready to proceed."

### 3. Execute Commands in Session
...
# 2. Wait for approval (ask user to attach)
